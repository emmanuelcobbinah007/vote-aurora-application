...
import { adminApi, Admin, AdminDashboardData, BulkInviteResult } from "@/services/adminApi";
...
export default function AdminManagementPage() {
  ...
  const [singleInviteForm, setSingleInviteForm] = useState({ email: "", full_name: "" });
  const [bulkInviteForm, setBulkInviteForm] = useState<BulkInviteForm>({ emails: "", full_names: "" });
  ...
  const handleBulkInvite = () => {
    const emails = bulkInviteForm.emails.split('\n').map(e => e.trim()).filter(Boolean);
    const full_names = bulkInviteForm.full_names.split('\n').map(n => n.trim()).filter(Boolean);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "ACTIVE":
        return <Badge variant="success" className="bg-green-100 text-green-800"><UserCheck className="w-3 h-3 mr-1" />Active</Badge>;
      case "SUSPENDED":
        return <Badge variant="destructive" className="bg-red-100 text-red-800"><UserX className="w-3 h-3 mr-1" />Suspended</Badge>;
      default:
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{dashboardData?.statistics.totalAdmins || 0}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-green-600">{dashboardData?.statistics.activeAdmins || 0}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{dashboardData?.statistics.totalAssignments || 0}</div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold text-orange-600">{dashboardData?.statistics.pendingInvitations || 0}</div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Latest administrator assignments to elections</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <div key={assignment.id} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm text-gray-600">
                        Assigned by {assignment.assigned_by} • {new Date(assignment.assigned_at).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                ))}
                {!dashboardData?.recentAssignments.length && (
                  <p className="text-center text-gray-500 py-4">No recent assignments</p>
                )}
              </div>
            </CardContent>
          </Card>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <p className="text-sm text-gray-600">{selectedAdmins.length} admin(s) selected</p>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {admin.assignments_count} assignment(s) • 
        ...
        <TabsContent value="activity" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Latest admin invitation activity</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {dashboardData?.recentInvitations.map((invitation) => (
                  <div key={invitation.id} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    <div className="text-right">
                      <Badge variant={invitation.used ? "success" : "secondary"}>
                ...
                {!dashboardData?.recentInvitations.length && (
                  <p className="text-center text-gray-500 py-4">No recent invitations</p>
                )}
              </div>
            </CardContent>
          </Card>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setSingleInviteForm({ ...singleInviteForm, full_name: e.target.value })}
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={inviteAdminMutation.isPending || !singleInviteForm.email || !singleInviteForm.full_name}
            >
      ...
      <Dialog open={bulkInviteModalOpen} onOpenChange={setBulkInviteModalOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Invite multiple administrators at once. Enter one email and name per line.
            </DialogDescription>
          </DialogHeader>
          ...
          {!bulkInviteResult ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => setBulkInviteForm({ ...bulkInviteForm, full_names: e.target.value })}
                    placeholder="John Doe&#10;Jane Smith&#10;Bob Johnson"
                    rows={6}
                  />
                </div>
            ...
            <div className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  Bulk invitation completed: {bulkInviteResult.summary.successful} successful, {bulkInviteResult.summary.failed} failed
                </AlertDescription>
              </Alert>
              ...
              {bulkInviteResult.failed_invitations.length > 0 && (
                <Card>
                  <CardHeader>
                    <CardTitle className="text-red-600">Failed Invitations</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      {bulkInviteResult.failed_invitations.map((failed, index) => (
                        <div key={index} className="flex justify-between items-center p-2 bg-red-50 rounded">
                          <span>{failed.email}</span>
                          <span className="text-sm text-red-600">{failed.error}</span>
                        </div>
                      ))}
                    </div>
                  </CardContent>
          ...
          <DialogFooter>
            ...
            {!bulkInviteResult && (
              <Button 
                ...
                disabled={bulkInviteMutation.isPending || !bulkInviteForm.emails || !bulkInviteForm.full_names}
              >
