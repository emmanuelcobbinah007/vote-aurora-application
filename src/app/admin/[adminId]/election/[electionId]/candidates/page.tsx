...
interface AdminCandidatesPageProps {
  params: Promise<{ adminId: string; electionId: string }>;
}
...
function AdminCandidatesClientPage({
  adminId,
  electionId,
}: {
  adminId: string;
  ...
  useEffect(() => {
    ...
    else if (session && session.user.role !== "ADMIN") router.push("/");
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Add, edit, or remove candidates for this election. All candidates
          require a name and must be associated with a portfolio.
        </p>
      </div>
      ...
      <div className="mt-8 bg-green-50 border border-green-200 rounded-lg p-4">
        <h3 className="font-medium text-green-800 mb-2">
          About Candidate Photos
        </h3>
        <p className="text-green-700 text-sm">
          Uploaded candidate photos are automatically stored in Cloudinary.
        </p>
      </div>
    </div>
  );
